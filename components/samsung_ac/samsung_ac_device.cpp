...
climate::ClimateTraits Samsung_AC_Climate::traits() {
  ...
  if (device->supports_fan_modes()) {
    traits.set_supported_fan_modes(
        {climate::CLIMATE_FAN_HIGH, climate::CLIMATE_FAN_MIDDLE, climate::CLIMATE_FAN_LOW, climate::CLIMATE_FAN_AUTO});
...

  {
    ...
    if (h || v) {
      ...
      if (h) traits.add_supported_swing_mode(climate::CLIMATE_SWING_HORIZONTAL);
      if (v) traits.add_supported_swing_mode(climate::CLIMATE_SWING_VERTICAL);
      if (h && v) traits.add_supported_swing_mode(climate::CLIMATE_SWING_BOTH);
    }
  }
...
void Samsung_AC_Climate::control(const climate::ClimateCall& call) {
  ...
  if (targetTempOpt.has_value()) request.target_temp = targetTempOpt.value();
...
void Samsung_AC_Climate::set_alt_mode_by_name(ProtocolRequest& request, const AltModeName& name) {
  auto supported = device->get_supported_alt_modes();
  auto mode =
      std::find_if(supported->begin(), supported->end(), [&name](const AltModeDesc& x) { return x.name == name; });
